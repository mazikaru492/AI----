...
/**
 * Canvas画像エディタ - GLM-OCRによる数値検出 + AI検証ループ
 */
export function CanvasImageEditor({
  ...
  // 画像をCanvasに読み込み
  const loadImage = useCallback(async (): Promise<{
    width: number;
    height: number;
  }> => {
   ...
   * GLM-OCRで数値を検出
   */
  const detectWithGlmOcr = useCallback(async () => {
    setState("loading-detect");
    setError(null);
    setStatusMessage("GLM-OCRで数値を検出中...");
    ...
    try {
      ...
      console.log("[Detect] Calling GLM-OCR API...");
      ...
      // スケールを適用してCanvas座標に変換
      const scaledNumbers = result.numbers.map((n) => ({
        text: n.text,
        bbox: {
          x: n.bbox.x * scaleRef.current,
          y: n.bbox.y * scaleRef.current,
          width: n.bbox.width * scaleRef.current,
          height: n.bbox.height * scaleRef.current,
        },
      }));

      console.log("[Detect] Found:", scaledNumbers.length, "numbers");
      setDetectedNumbers(scaledNumbers);
      setState("detect-complete");
      setStatusMessage("");
    } catch (e) {
      console.error("[Detect] Error:", e);
      setError(e instanceof Error ? e.message : "検出エラー");
      setState("error");
      setStatusMessage("");
    }
  }, [loadImage]);
  ...
  // Canvasに数値を描画
  const drawReplacementsOnCanvas = useCallback(
    (replacementsWithBbox: NumberReplacement[]) => {
    ...
    try {
      ...
      while (!isValid && attempts < MAX_RETRY_ATTEMPTS) {
        ...
        setVerificationStatus(
          `試行 ${attempts}/${MAX_RETRY_ATTEMPTS}: 数値を生成中...`,
        );
        ...
        setState("verifying");
        setVerificationStatus(
          `試行 ${attempts}/${MAX_RETRY_ATTEMPTS}: AIが検証中...`,
        );
        ...
        <div className="rounded-lg bg-blue-50 p-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-blue-900">
              GLM-OCRで {detectedNumbers.length}個の数値を検出しました
            </p>
          </div>
      ...
      {state === "complete" && replacements.length > 0 && (
        <div className="rounded-lg bg-zinc-50 p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {[
              ...new Map(replacements.map((r) => [r.original, r])).values(),
            ].map((r, i) => (
              <span
                key={i}
